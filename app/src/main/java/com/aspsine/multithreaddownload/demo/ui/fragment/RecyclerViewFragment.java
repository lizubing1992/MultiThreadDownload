...
import com.aspsine.multithreaddownload.demo.ui.adapter.RecyclerViewAdapter.AppViewHolder;
...
public class RecyclerViewFragment extends Fragment implements OnItemClickListener<AppInfo> {
  ...
  private void download(final int position, String tag, final AppInfo appInfo) {
    final DownloadRequest request = new Builder()
  ...
  class DownloadCallback implements CallBack {
    ...
    public void onConnecting() {
      ...
      if (isCurrentListViewItemVisible(mPosition)) {
        AppViewHolder holder = getViewHolder(mPosition);
  ...
  private AppViewHolder getViewHolder(int position) {
    return (AppViewHolder) recyclerView.findViewHolderForLayoutPosition(position);
  }
  ...
  private String getDownloadPerSize(long finished, long total) {
    return DF.format((float) finished / (1024 * 1024)) + "M/" + DF
        .format((float) total / (1024 * 1024)) + "M";
  }
}
